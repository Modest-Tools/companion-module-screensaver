...
/**
 * Load a full-deck animated GIF as a master screensaver. Pre-decodes all
 * frames into RGBA buffers using the running-canvas disposal handling pattern
 * (same as `gifFrames.ts`), so `getFrameRgba` is an O(1) lookup at playback
 * time. Memory cost: width × height × 4 × numFrames bytes — fine for the
 * full-deck animation sizes Elgato ships (typically <50 MB resident).
 */
export async function loadGifMaster(filePath: string, gridCols: number, gridRows: number): Promise<MasterDeck> {
	...
	const dims = computeTileDims(width, height, gridCols, gridRows)
